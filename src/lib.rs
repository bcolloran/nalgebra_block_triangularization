...
/// Return row/column permutations P, Q (as PermutationSequence) such that:
///     L = P * mat * Q
/// is (lower) block triangular with respect to the SCC block structure induced by a
/// maximum matching.
///
/// Notes:
/// - This is purely structural: it uses mat[(i,j)] != Default::default() as "nonzero".
...
///
/// You apply these like:
///   let (pr, pc) = lower_triangular_permutations(&mat);
///   let mut l = mat.clone();
    ...
    S: Storage<T, R, C>,
{
    ...
    // Topologically order SCC DAG so edges go "forward", then reverse for lower triangular.
